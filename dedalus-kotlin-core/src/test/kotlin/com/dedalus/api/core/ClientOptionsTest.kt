...
internal class ClientOptionsTest {
    ...
    @Test
    fun toBuilder_dedalusOrgIdCanBeUpdated() {
        var clientOptions =
            ClientOptions.builder()
                .httpClient(httpClient)
                .dedalusOrgId("My Dedalus Org ID")
                .apiKey("My API Key")
                .build()

        clientOptions = clientOptions.toBuilder().dedalusOrgId("another My Dedalus Org ID").build()

        assertThat(clientOptions.headers.values("X-Dedalus-Org-Id"))
            .containsExactly("another My Dedalus Org ID")
    }
    ...
    fun toBuilder_apiKeyAuthCanBeUpdated() {
        ...
        clientOptions = clientOptions.toBuilder().xApiKey("another My X API Key").build()

        assertThat(clientOptions.headers.values("x-api-key"))
            .containsExactly("another My X API Key")
    }
